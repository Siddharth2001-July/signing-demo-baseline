...
export const SignDemo: React.FC<{ allUsers: User[]; user: User }> = ({
  ...
  const deleteUser = (user: User) => {
    ...
    let currSig = users.find((userL)=>(userL.role!=='Editor'));
    setUsers(remainingUsers);
    //@ts-ignore
    setCurrSignee(currSig);
  }
            ...
            <>
              ...
              <div style={{ padding: "16px", borderBottom: "1px solid #D7DCE4" }}>
                ...
                <div style={{
                  ...
                  lineHeight: "14px",
                  marginBottom: "10px"
                }}>Select a signee for whom you need to add fields</div>
                <Checkbox
                  label="Ready to sign"
                    ...
                    {users?.map((user) => {
                      ...
                      return (
                        <div className="heading-custom-style_hover" key={user?.id} onClick={() => signeeChanged(user)}>
                          ...
                          {user?.name}
                          <span className="cross" onClick={() => deleteUser(user)}>
